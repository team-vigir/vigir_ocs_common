...
/*
  ...
  Ignoring effort as I do not want to scale markers.
...
namespace rviz
{
    ...
    JointInfo* JointMarkerDisplayCustom::getJointInfo( const std::string& joint)
    {
    ...
    JointInfo* JointMarkerDisplayCustom::createJoint(const std::string &joint)
    {
    ...
    void JointMarkerDisplayCustom::processMessage( const sensor_msgs::JointState::ConstPtr& msg )
    {
        ...
        boost::shared_ptr<EffortVisualCustom> visual;
        if (visuals_.full())
        {
        ...
        {
            visual.reset(new EffortVisualCustom(context_->getSceneManager(), scene_node_, robot_model_));
        }
...
//        Ogre::Quaternion* rApplied = new Ogre::Quaternion(1.0,0.01f,0,0);
//        joint_orientation = joint_orientation * *rApplied;
