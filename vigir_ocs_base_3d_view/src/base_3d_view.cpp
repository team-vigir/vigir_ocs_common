    ...
    , visualize_grid_map_(true)
{
    ...
    // Check if this is going to be a shared context or not
    if(context != NULL)
    {
        manager_ = context;
    ...
    else // need to create everything
    {
        ...
        ROS_ASSERT( footsteps_array_ != NULL );
        ...
        ROS_ASSERT( planner_start_ != NULL );
    ...
    // Connect template interactive markers to the display
    ...
    // initialize status bar/reset view button
    ...
    //ground_map_sub_ = nh_.subscribe<nav_msgs::OccupancyGrid>( "/flor/worldmodel/grid_map_near_robot", 5, &Base3DView::processNewMap, this );
